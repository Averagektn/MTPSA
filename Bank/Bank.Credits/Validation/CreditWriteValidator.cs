...
public sealed class CreditWriteValidator(
    CreditsDbContext db,
    IValidator<CreditContractRequest> validator,
    IClientsApi clientsApi)
{
    public async Task<Result> ValidateAsync(CreditContractRequest request, CancellationToken cancellationToken = default)
    {
        request.Normalize();
        ...
        if (!string.IsNullOrWhiteSpace(request.Number)
            && await db.CreditContracts.AnyAsync(c => c.Number == request.Number, cancellationToken))
        {
