...
import { createContract, fetchBankingDay, fetchCreditDictionaries } from './api';
import { useLocale } from './i18n/LocaleContext';
import { translateError } from './i18n/translate';
import { ScheduleTable } from './ScheduleTable';
import type { ClientListItem, CreditDictionaries, CreditProduct, FieldErrors } from './types';
import {
  applyProduct,
  buildSchedule,
  emptyContractForm,
...
export function ContractForm({ onCancel, onSaved }: ContractFormProps) {
  ...
  const [dictionaries, setDictionaries] = useState<CreditDictionaries | null>(null);
  ...
  const selectedProduct = useMemo<CreditProduct | undefined>(
  ...
  const previewSchedule = useMemo(() => {
    ...
    return buildSchedule(amount, selectedProduct.annualRate, startIso, selectedProduct.termMonths, selectedProduct.repaymentSchedule);
  }, [selectedProduct, values.amount, values.startDate]);
    ...
    <form className="client-form" onSubmit={(event) => void onSubmit(event)} noValidate>
      ...
      {previewSchedule.length > 0 ? (
        <fieldset>
          ...
          {selectedProduct ? (
            <p className="muted">{selectedProduct.repaymentSchedule === 'interestOnly' ? t.interestOnly : t.annuity}</p>
          ) : null}
          <ScheduleTable items={previewSchedule} />
        </fieldset>
      ) : null}
