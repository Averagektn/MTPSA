...
[TestClass]
[DoNotParallelize]
public sealed class DepositWebDriverTests : WebDriverTestBase
{
    ...
    {
        _host = await DepositsTestHost.StartAsync(withFrontend: true, clientsApi: FakeClientsApi.Seeded());
    ...
    public void Creating_a_contract_shows_it_in_the_list()
    {
        ...
        Select("clientId", "1");
        Select("productId", "1");
        Type("number", "UI-SAFE-1");
        Type("amount", "1000");
        _driver.FindElement(By.CssSelector("[data-testid='save-contract']")).Click();
        _wait.Until(d => d.FindElements(By.CssSelector("[data-testid='contract-row']")).Count >= 3);
        _driver.PageSource.Should().Contain("UI-SAFE-1");
    }

    [TestMethod]
    public void Accounts_report_shows_cash_and_development_fund()
    {
        _driver.Navigate().GoToUrl($"{_host.BaseUrl}/accounts");
        _wait.Until(d => d.FindElements(By.CssSelector("[data-testid='account-row']")).Count >= 6);
        _driver.PageSource.Should().Contain(Bank.Deposits.Accounting.BankAccountNumbers.Cash);
        _driver.PageSource.Should().Contain(Bank.Deposits.Accounting.BankAccountNumbers.DevelopmentFund);
    }
